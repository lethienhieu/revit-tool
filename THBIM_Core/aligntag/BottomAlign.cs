...
namespace THBIM
{
    ...
    public class AlignTagsBottomCommand : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData cd, ref string msg, ElementSet set)
        {
            ...
            if (tags.Count < 2)
            {
                TaskDialog.Show("Align Bottom", "Cần quét chọn ít nhất 2 Tag.");
                return Result.Cancelled;
            }

            XYZ up = view.UpDirection;

            // Chuẩn bị dữ liệu: key point + toạ độ y theo UpDirection
            ...
            foreach (var tag in tags)
            {
                ...
                if (p == null) continue;
                double y = Dot(p, up);
                infos.Add(new TagInfo(tag, p, y));
            }

            if (infos.Count < 2)
            {
                TaskDialog.Show("Align Bottom", "Không xác định được vị trí Tag.");
                return Result.Cancelled;
            }

            // Anchor = tag có y nhỏ nhất (dưới cùng)
            TagInfo anchor = infos.OrderBy(i => i.AxisValue).First();
            double yAnchor = anchor.AxisValue;

            int moved = 0, skippedPinned = 0, skippedTol = 0, failed = 0;

            using (Transaction t = new Transaction(doc, "THBIM - Align Tags Bottom"))
            {
                t.Start();
                ...
                foreach (var info in infos)
                {
                    ...
                    double dy = yAnchor - info.AxisValue;
                    if (Math.Abs(dy) <= TOL_FT)
                    {
                        skippedTol++;
                        continue;
                    }
        ...
        private static List<IndependentTag> GetTagsFromSelectionOrPick(UIDocument uidoc, Document doc, View view)
        {
            ...
            try
            {
                IList<Element> rect = uidoc.Selection.PickElementsByRectangle(
                    new TagSelectionFilter(), "Quét chọn các Tag để căn BOTTOM");
        ...
        private class TagInfo
        {
            ...
            public double AxisValue { get; } // y theo UpDirection

            public TagInfo(IndependentTag tag, XYZ point, double axisVal)
            {
                Tag = tag; Point = point; AxisValue = axisVal;
            }
        }
