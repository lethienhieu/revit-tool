...
namespace THBIM
{
    // Class này không cần kế thừa IExternalCommand nữa, nó chỉ là một class xử lý logic
    public class Floordropright
    {
        public Result Run(UIDocument uidoc, FamilySymbol symbol, ref string message)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    // --- LOGIC: Lấy giá trị chuỗi (Số hoặc "Var.") ---
                    string offsetText = GetAdjacentFloorText(doc, pickedPoint);

                    // Nếu có giá trị hợp lệ (khác null) thì mới đặt Family
                    if (!string.IsNullOrEmpty(offsetText))
                    {
                        using (Transaction t = new Transaction(doc, "Place & Align & Orient"))
                        {
                            t.Start();
        ...
        // ==========================================================
        // KHU VỰC CÁC HÀM HỖ TRỢ (GIỮ NGUYÊN LOGIC CŨ)
        // ==========================================================
        ...
        private void ProcessOrientationByAngle(Document doc, FamilyInstance instance, Reference edgeRef, XYZ pickPoint)
        {
            ...
            if (isGreaterThan90)
            {
                // CASE: Góc > 90 độ -> Xoay 180 trước, Mirror sau
            ...
            {
                // CASE: Góc <= 90 độ (Bao gồm trục hoành)
                bool isHorizontal = Math.Abs(lineDir.Y) < 0.001;

                if (isHorizontal)
                {
                    // Trục Hoành: Xoay 180 trước -> Mirror Trên/Dưới sau
                    Line axis = Line.CreateBound(pickPoint, pickPoint + XYZ.BasisZ);
                    ElementTransformUtils.RotateElement(doc, instance.Id, axis, Math.PI);
                    doc.Regenerate();
                ...
                {
                    // Các trục khác (< 90 độ): Chỉ Mirror Trái/Phải
                    CheckAndMirrorStandard(doc, instance, pickPoint, lineDir);
                }
            }
        }
