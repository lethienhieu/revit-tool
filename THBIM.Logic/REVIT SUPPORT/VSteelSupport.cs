...
{
    public static class VSteelSupport
    {
        public static void Run(ExternalCommandData commandData, Document doc, List<Element> elements, double offsetA, double spacingB)
        {
            ...
            FamilySymbol symbol = new FilteredElementCollector(doc)
                ...
                .Cast<FamilySymbol>()
                .FirstOrDefault(s => s.Family.Name == "TH_DA_V_Steel_Thread Rod");

            if (symbol == null)
            {
                TaskDialog.Show("Error", "Family 'TH_DA_V_Steel_Thread Rod' not found");
                return;
            }
            ...
            // NEW: store elevation difference to write to param "LAYER 2"
            double diffZFeet = 0.0; // feet
            double diffZmm = 0.0;   // mm
            ...
            using (Transaction trans = new Transaction(doc, "Place VSteel Supports"))
            {
                trans.Start();
                ...
                foreach (var r in pickedRefs)
                {
                    ...
                    while (currentDist < length)
                    {
                        ...
                        // NEW: Write elevation difference value to parameter "LAYER 2"
                        try
                        {
                        ...
                        try
                        {
                            ...
                            if (mepCurve != null && mepCurve.MEPSystem != null)
                            {
                                ElementId typeId = mepCurve.MEPSystem.GetTypeId();
                                if (typeId != ElementId.InvalidElementId)
                                {
                                    Element sysTypeElem = doc.GetElement(typeId);
                                    systemType = sysTypeElem?.Name; // Name of System Type
                                }
                            }

                            // Fallback: read built-in parameter displaying type name (AsValueString)
                            if (string.IsNullOrEmpty(systemType))
                            {
                                BuiltInParameter bip = (elem is Pipe)
                                    ? BuiltInParameter.RBS_PIPING_SYSTEM_TYPE_PARAM
                                    : BuiltInParameter.RBS_DUCT_SYSTEM_TYPE_PARAM;
                                Parameter pSys = elem.get_Parameter(bip);
                                if (pSys != null)
                                    systemType = pSys.AsValueString();
                            }
